...
public class ZiXunFragment extends Fragment {
    ...
    private void onLoadMore() {
        ...
        new HttpUtils<List<HomePublishBean>>(type, HttpConstant.API_HOME_ACTIVITIES, loadMoreParams, "更多资讯返回数据", false) {
            @Override
    ...
    private void getPublish() {
        ...
        hotGameParams.put("category", "1");
        Type type = new TypeToken<HttpResult<List<HomePublishBean>>>() {
        }.getType();
        if (springview!=null){
            springview.onFinishFreshAndLoad();
        }
        new HttpUtils<List<HomePublishBean>>(type, HttpConstant.API_HOME_ACTIVITIES, hotGameParams, "获取资讯", false) {
            @Override
            protected void _onSuccess(List<HomePublishBean> listData, String msg) {
                if (listData.size() <= 0) {
                    llPublishServer.setVisibility(View.VISIBLE);
